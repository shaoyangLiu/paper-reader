@echo off
REM Paper Reader - 一键修复 (kill stale servers on port 8731 + restart + health-check + open)
chcp 65001 >nul 2>&1
...
echo [3/4] 健康检查（确认服务器真的在响应，而非仅仅占着端口）...
set "OK=0"
...
echo.
echo 完成！浏览器应已打开阅读器。
echo 若仍打不开，请查看 "PaperReader-Server" 窗口里的报错。
echo.
pause >nul
